...
function useFetcher({
  ...
  let route2 = reactExports.useContext(RouteContext);
  let routeId = (_a = route2.matches[route2.matches.length - 1]) == null ? void 0 : _a.route.id;
  invariant(fetcherData, `useFetcher must be used inside a FetchersContext`);
  invariant(route2, `useFetcher must be used inside a RouteContext`);
  invariant(
    routeId != null,
  ...
  let FetcherForm = reactExports.useMemo(() => {
    let FetcherForm2 = reactExports.forwardRef(
      (props, ref2) => {
        return /* @__PURE__ */ reactExports.createElement(Form, { ...props, navigate: false, fetcherKey, ref: ref2 });
      }
    );
...
var unsign = async (cookie2, secret) => {
  let index = cookie2.lastIndexOf(".");
  let value = cookie2.slice(0, index);
  let hash = cookie2.slice(index + 1);
  let data2 = encoder.encode(value);
...
async function encodeCookieValue(value, secrets) {
  ...
  if (secrets.length > 0) {
    encoded = await sign$1(encoded, secrets[0]);
  }
  return encoded;
}
...
function warnOnceAboutSigningSessionCookie(cookie2) {
  warnOnce(
    cookie2.isSigned,
    `The "${cookie2.name}" cookie is not signed, but session cookies should be signed to prevent tampering on the client before they are sent back to the server. See https://remix.run/utils/cookies#signing-cookies for more information.`
  );
}
function createCookieSessionStorage({ cookie: cookieArg } = {}) {
  let cookie2 = isCookie(cookieArg) ? cookieArg : createCookie((cookieArg == null ? void 0 : cookieArg.name) || "__session", cookieArg);
  warnOnceAboutSigningSessionCookie(cookie2);
  return {
    async getSession(cookieHeader, options) {
      return createSession(
        cookieHeader && await cookie2.parse(cookieHeader, options) || {}
      );
    },
    async commitSession(session, options) {
      let serializedCookie = await cookie2.serialize(session.data, options);
...
var StreamPump = class {
  constructor(stream2) {
    ...
    this.highWaterMark = stream2.readableHighWaterMark || new Stream.Readable().readableHighWaterMark;
...
function requireStreams() {
  ...
  var Buffer2 = require$$0$7.Buffer, Transform = stream.Transform;
...
function requireStringify() {
  ...
  var sentinel = {};
  var stringify2 = function stringify3(object, prefix, generateArrayPrefix, commaRoundTrip, allowEmptyArrays, strictNullHandling, skipNulls, encodeDotInKeys, encoder2, filter, sort, allowDots, serializeDate, format, formatter, encodeValuesOnly, charset2, sideChannel2) {
    var obj = object;
    ...
    if (obj === null) {
      if (strictNullHandling) {
        return encoder2 && !encodeValuesOnly ? encoder2(prefix, defaults.encoder, charset2, "key", format) : prefix;
      }
      obj = "";
    }
    if (isNonNullishPrimitive(obj) || utils2.isBuffer(obj)) {
      if (encoder2) {
        var keyValue = encodeValuesOnly ? prefix : encoder2(prefix, defaults.encoder, charset2, "key", format);
        return [formatter(keyValue) + "=" + formatter(encoder2(obj, defaults.encoder, charset2, "value", format))];
      }
...
function requireSend() {
  ...
  SendStream.prototype.redirect = function redirect2(path2) {
    var res = this.res;
  ...
  SendStream.prototype.stream = function stream2(path2, options) {
    var self2 = this;
    var res = this.res;
    var stream3 = fs.createReadStream(path2, options);
...
function requireResponse() {
  ...
  res.redirect = function redirect2(url) {
    var address = url;
    var body;
    var status = 302;
    if (arguments.length === 2) {
...
function requireServeStatic() {
  ...
  function serveStatic$1(root, options) {
    ...
    var redirect2 = opts.redirect !== false;
    ...
    var onDirectory = redirect2 ? createRedirectDirectoryListener() : createNotFoundDirectoryListener();
    return function serveStatic2(req, res, next) {
      ...
      var stream2 = send2(req, path, opts);
...
app.use(
  createRequestHandler({
    ...
    build: () => import("./server-build-DF8Nzo8p.js"),
    getLoadContext() {
      return {
...
export {
  ...
  React$1 as h,
  useNavigation as i,
  requireMimeTypes as j,
  requireSrc as k,
  createCookieSessionStorage as l,
  Links as m,
  ScrollRestoration as n,
  Scripts as o,
  isRouteErrorResponse as p,
  useFetcher as q,
  requireReact as r,
  data as s,
  redirect as t,
  useParams as u,
  useOutletContext as v,
  useNavigate as w,
